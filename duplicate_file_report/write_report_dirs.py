...
def transform(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    dirs = df_tmp.groupby(by=["hash_set"]).apply(lambda r: frozenset(r["parent_dir"].tolist()))
    ...
    df_res["list_duplicated_dirs"] = df_res.apply(lambda r: list(r["duplicated_dirs"]), axis=1)
